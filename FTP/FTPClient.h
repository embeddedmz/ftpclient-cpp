...
public:
  ...
  void SetProgressFnCallback(void *pOwner,
  ...
  // Session
  const bool InitSession(const std::string &strHost, const unsigned &uPort,
  ...
  const bool List(const std::string &strRemoteFolder, std::string &strList,
                  bool bOnlyNames = true) const;

  const bool DownloadFile(const std::string &strLocalFile,
                          const std::string &strRemoteFile) const;

  const bool DownloadFile(const std::string &strRemoteFile,
...
private:
  ...
  // Curl callbacks
  static size_t WriteInStringCallback(void *ptr, size_t size, size_t nmemb,
                                      void *data);
  static size_t WriteToFileCallback(void *ptr, size_t size, size_t nmemb,
                                    void *data);
  static size_t ReadFromFileCallback(void *ptr, size_t size, size_t nmemb,
                                     void *stream);
  static size_t ThrowAwayCallback(void *ptr, size_t size, size_t nmemb,
                                  void *data);
  ...
  static size_t WriteItCallback(char *buff, size_t size, size_t nmemb,
                                void *cb_data);

  // String Helpers
  static std::string StringFormat(std::string strFormat, ...);
  static void ReplaceString(std::string &strSubject,
                            const std::string &strSearch,
                            const std::string &strReplace);

// Curl Debug informations
#ifdef DEBUG_CURL
  static int DebugCallback(CURL *curl, curl_infotype curl_info_type,
                           char *strace, size_t nSize, void *pFile);
...
#define LOG_ERROR_CURL_REMOVE_FORMAT                                           \
  "[FTPClient][Error] Unable to remove file %s (Error = %d | %s)."
#define LOG_ERROR_CURL_VERIFYURL_FORMAT                                        \
  "[FTPClient][Error] Unable to connect to the remote folder %s (Error = %d "  \
  "| %s)."
#define LOG_ERROR_CURL_FILETIME_FORMAT                                         \
  "[FTPClient][Error] Unable to get file %s's info (Error = %d | %s)."
#define LOG_ERROR_CURL_GETFILE_FORMAT                                          \
  "[FTPClient][Error] Unable to import remote File %s/%s (Error = %d | %s)."
#define LOG_ERROR_CURL_UPLOAD_FORMAT                                           \
  "[FTPClient][Error] Unable to upload file %s (Error = %d | %s)."
#define LOG_ERROR_CURL_FILELIST_FORMAT                                         \
  "[FTPClient][Error] Unable to connect to remote folder %s (Error = %d | "    \
  "%s)."
#define LOG_ERROR_CURL_GETWILD_FORMAT                                          \
  "[FTPClient][Error] Unable to import elements %s/%s (Error = %d | %s)."
#define LOG_ERROR_CURL_GETWILD_REC_FORMAT                                      \
  "[FTPClient][Error] Encountered a problem while importing %s to %s."
#define LOG_ERROR_CURL_MKDIR_FORMAT                                            \
  "[FTPClient][Error] Unable to create directory %s (Error = %d | %s)."
#define LOG_ERROR_CURL_RMDIR_FORMAT                                            \
  "[FTPClient][Error] Unable to remove directory %s (Error = %d | %s)."
