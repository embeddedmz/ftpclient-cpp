...
CFTPClient::CFTPClient(LogFnCallback Logger)
    : m_oLog(std::move(Logger)), m_iCurlTimeout(0), m_uPort(0),
      m_eFtpProtocol(FTP_PROTOCOL::FTP), m_bActive(false), m_bNoSignal(false),
      m_bProgressCallbackSet(false), m_eSettingsFlags(ALL_FLAGS),
      m_pCurlSession(nullptr), m_curlHandle(CurlHandle::instance()) {
  if (!m_oLog) {
 ...
 */
const bool
CFTPClient::InitSession(const std::string &strHost, const unsigned &uPort,
 ...
 *
 */
void CFTPClient::SetProgressFnCallback(void *pOwner,
                                       const ProgressFnCallback &fnCallback) {
...
const bool CFTPClient::CreateDir(const std::string &strNewDir) const {
  ...
  curl_easy_setopt(m_pCurlSession, CURLOPT_FTP_CREATE_MISSING_DIRS,
                   CURLFTP_CREATE_DIR);
  ...
  if (res != CURLE_OK) {
    if (m_eSettingsFlags & ENABLE_LOG)
      m_oLog(StringFormat(LOG_ERROR_CURL_MKDIR_FORMAT,
                          strRemoteNewFolderName.c_str(), res,
...
const bool CFTPClient::RemoveDir(const std::string &strDir) const {
  ...
  if (res != CURLE_OK) {
    if (m_eSettingsFlags & ENABLE_LOG)
      m_oLog(StringFormat(LOG_ERROR_CURL_RMDIR_FORMAT,
                          strRemoteFolderName.c_str(), res,
                          curl_easy_strerror(res)));
  } else
    bRet = true;
...
const bool CFTPClient::RemoveFile(const std::string &strRemoteFile) const {
  ...
  if (res != CURLE_OK) {
    if (m_eSettingsFlags & ENABLE_LOG)
      m_oLog(StringFormat(LOG_ERROR_CURL_REMOVE_FORMAT, strRemoteFile.c_str(),
 ...
 * @endcode
 */
const bool CFTPClient::Info(const std::string &strRemoteFile,
  ...
  curl_easy_setopt(m_pCurlSession, CURLOPT_URL,
                   ParseURL(strRemoteFile).c_str());
  ...
  if (CURLE_OK == res) {
    ...
    res = curl_easy_getinfo(m_pCurlSession, CURLINFO_CONTENT_LENGTH_DOWNLOAD,
                            &oFileInfo.dFileSize);
 ...
 * @endcode
 */
const bool CFTPClient::DownloadFile(const std::string &strLocalFile,
  ...
  ofsOutput.open(strLocalFile, std::ofstream::out | std::ofstream::binary |
                                   std::ofstream::trunc);
 ...
 * more information.
 */
const bool CFTPClient::DownloadFile(const std::string &strRemoteFile,
 ...
 */
const bool
CFTPClient::DownloadWildcard(const std::string &strLocalDir,
...
#ifdef LINUX
  data.strOutputPath =
      strLocalDir + ((strLocalDir[strLocalDir.length() - 1] != '/') ? "/" : "");
#else
  data.strOutputPath =
      strLocalDir +
      ((strLocalDir[strLocalDir.length() - 1] != '\\') ? "\\" : "");
#endif

  std::string strPattern = ParseURL(strRemoteWildcard);

  struct stat info;
  if (stat(data.strOutputPath.c_str(), &info) == 0 &&
      (info.st_mode & S_IFDIR)) // S_ISDIR() doesn't exist on windows
  {
    ...
    curl_easy_setopt(m_pCurlSession, CURLOPT_CHUNK_BGN_FUNCTION,
                     FileIsComingCallback);
    ...
    curl_easy_setopt(m_pCurlSession, CURLOPT_CHUNK_END_FUNCTION,
                     FileIsDownloadedCallback);
    ...
    if (res != CURLE_OK && res != CURLE_REMOTE_FILE_NOT_FOUND) {
      if (m_eSettingsFlags & ENABLE_LOG)
        m_oLog(StringFormat(LOG_ERROR_CURL_GETWILD_FORMAT, m_strServer.c_str(),
                            strRemoteWildcard.c_str(), res,
                            curl_easy_strerror(res)));
    }
    /* folders need to be copied integrally */
    else if (!data.vecDirList.empty() &&
             strRemoteWildcard[strRemoteWildcard.length() - 1] == '*') {
      std::string strBaseUrl =
          strRemoteWildcard.substr(0, strRemoteWildcard.length() - 1);
      ...
      for (const auto &Dir : data.vecDirList) {
        if (!DownloadWildcard(data.strOutputPath + Dir,
                              strBaseUrl + Dir + "/*")) {
    ...
    m_oLog(
        StringFormat(LOG_ERROR_DIR_GETWILD_FORMAT, data.strOutputPath.c_str()));
...
const bool CFTPClient::UploadFile(const std::string &strLocalFile,
  ...
  if (stat(strLocalFile.c_str(), &file_info) == 0) {
    ...
    if (!InputFile) {
      if (m_eSettingsFlags & ENABLE_LOG)
        m_oLog(
            StringFormat(LOG_ERROR_FILE_UPLOAD_FORMAT, strLocalFile.c_str()));
    ...
    curl_easy_setopt(m_pCurlSession, CURLOPT_READFUNCTION,
                     ReadFromFileCallback);
    ...
    curl_easy_setopt(m_pCurlSession, CURLOPT_INFILESIZE_LARGE,
                     static_cast<curl_off_t>(file_info.st_size));
    ...
    if (res != CURLE_OK) {
      if (m_eSettingsFlags & ENABLE_LOG)
        m_oLog(StringFormat(LOG_ERROR_CURL_UPLOAD_FORMAT, strLocalFile.c_str(),
                            res, curl_easy_strerror(res)));
    } else
      bRes = true;
  }
...
const CURLcode CFTPClient::Perform() const {
  ...
  curl_easy_setopt(m_pCurlSession, CURLOPT_USERPWD,
                   (m_strUserName + ":" + m_strPassword).c_str());
  ...
  if (m_bProgressCallbackSet) {
    curl_easy_setopt(m_pCurlSession, CURLOPT_PROGRESSFUNCTION,
                     *GetProgressFnCallback());
  ...
  if (m_eFtpProtocol == FTP_PROTOCOL::FTPS ||
      m_eFtpProtocol == FTP_PROTOCOL::FTPES)
  ...
  if (m_eFtpProtocol == FTP_PROTOCOL::SFTP && m_eSettingsFlags & ENABLE_SSH)
    ...
    curl_easy_setopt(m_pCurlSession, CURLOPT_SSH_AUTH_TYPES,
                     CURLSSH_AUTH_AGENT);
...
std::string CFTPClient::StringFormat(std::string strFormat, ...) {
  int n = (static_cast<int>(strFormat.size())) *
          2; // Reserve two times as much as the length of the strFormat
...
void CFTPClient::ReplaceString(std::string &strSubject,
...
size_t CFTPClient::ThrowAwayCallback(void *ptr, size_t size, size_t nmemb,
                                     void *data) {
 ...
 * @return (size * nmemb)
 */
size_t CFTPClient::WriteInStringCallback(void *ptr, size_t size, size_t nmemb,
                                         void *data) {
 ...
 * @return (size * nmemb)
 */
size_t CFTPClient::WriteToFileCallback(void *buff, size_t size, size_t nmemb,
                                       void *data) {
 ...
 * @return (size * nmemb)
 */
size_t CFTPClient::WriteToMemory(void *buff, size_t size, size_t nmemb,
                                 void *data) {
  if ((size == 0) || (nmemb == 0) || ((size * nmemb) < 1) || (data == nullptr))
    return 0;
 ...
 * @return (size * nmemb)
 */
size_t CFTPClient::ReadFromFileCallback(void *ptr, size_t size, size_t nmemb,
                                        void *stream) {
 ...
 * (continue)
 */
long CFTPClient::FileIsComingCallback(struct curl_fileinfo *finfo,
                                      WildcardTransfersCallbackData *data,
                                      int remains) {
...
#ifdef LINUX
    if (mkdir((data->strOutputPath + finfo->filename).c_str(), ACCESSPERMS) !=
            0 &&
        errno != EEXIST)
#else
    if (_mkdir((data->strOutputPath + finfo->filename).c_str()) != 0 &&
        errno != EEXIST)
#endif
    {
      std::cerr << "Problem creating directory (errno=" << errno
                << "): " << data->strOutputPath + finfo->filename << std::endl;
  ...
  case CURLFILETYPE_FILE:
    ...
    //}
    data->ofsOutput.open(data->strOutputPath + finfo->filename,
 ...
 */
size_t CFTPClient::WriteItCallback(char *buff, size_t size, size_t nmemb,
                                   void *cb_data) {
  WildcardTransfersCallbackData *data =
      reinterpret_cast<WildcardTransfersCallbackData *>(cb_data);
  size_t written = 0;
...
#ifdef WINDOWS
      && s_strCurlTraceLogDirectory.at(s_strCurlTraceLogDirectory.length() -
                                       1) != '\\') {
...
int CFTPClient::DebugCallback(CURL *curl, curl_infotype curl_info_type,
                              char *pszTrace, size_t usSize, void *pFile) {
  std::string strText;
...
void CFTPClient::StartCurlDebug() const {
  if (!m_ofFileCurlTrace.is_open()) {
    ...
    if (!strFileCurlTraceFullName.empty()) {
      ...
      m_ofFileCurlTrace.open(strFileCurlTraceFullName,
